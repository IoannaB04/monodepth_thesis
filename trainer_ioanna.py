...
class Trainer:
    def __init__(self, options):
        ...
        self.device = torch.device(f"cuda:{options.gpu}" if torch.cuda.is_available() and not options.no_cuda else "cpu")
        # torch.device("cpu" if self.opt.no_cuda else "cuda")
        ...
        print("\n\n\nTraining model named: ", self.opt.model_name)
        ...
        transforms_train = Compose([
            Resize(self.opt.height, self.opt.width),
        ...
        train_dataset = NuScenesDataset(nusc, version, split='train',
        ...
        val_dataset = NuScenesDataset(nusc, version, split='validation',
                                      height = self.opt.height,
                                      width = self.opt.width,
                                      scales = self.opt.scales,
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                cam_points = self.backproject_depth[source_scale](depth, inputs[("inv_K", source_scale)])  # [B, 4, N]
                cam_points_transformed = torch.matmul(T, cam_points)  # [B, 4, N]
                cam_points_xyz = cam_points_transformed[:, :3, :]     # [B, 3, N]
                pix_coords = torch.matmul(inputs[("K", source_scale)], cam_points_xyz) # [B, 3, N]

                pix_coords_norm = pix_coords[:, :2, :] / (pix_coords[:, 2:3, :] + 1e-7)  # [B,2,N]
                N, _, HW = pix_coords_norm.shape
                H, W = self.opt.height, self.opt.width

                x_norm = 2 * (pix_coords_norm[:, 0, :] / (W - 1)) - 1
                y_norm = 2 * (pix_coords_norm[:, 1, :] / (H - 1)) - 1
                grid = torch.stack((x_norm, y_norm), dim=2)  # [B, HW, 2]
                grid = grid.view(N, H, W, 2)  # reshape

                outputs[("sample", frame_id, scale)] = grid
